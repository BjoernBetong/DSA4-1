// Skript zum Bezahlen bzw. Geld bekommen
...
//###############################################################################################################################################################################################################################################    
	...
	headerDialog = "<h2>Geldbeutel/Geldkatze von <i>" + tokenName + "</i></h2>";
	...
	inputDialog += divFlexStart + "Kreuzer: <input id='inputKreuzer'" + divInputNumber  + "0'/>" + divFlexEnd;
...
//###############################################################################################################################################################################################################################################    
    ...
    async function htmlCallback(html){
        ...
        flavor = "Geldbeutel/Geldkatze<br>";
		if(Pay === -1){
			flavor += "<b>" + tokenName + "</b> bezahlt: <br>";
		}else{
		...
		flavor += hr;
		flavor += "Das aktuelle Vermögen beträgt:<br>";
		if(newDukaten === 1){
			flavor += "<b>" + newDukaten + "</b> Dukate<br>";
		}else{
			flavor += "<b>" + newDukaten + "</b> Dukaten<br>";
		}
		flavor += "<b>" + newSilber + "</b> Silber<br>";
		flavor += "<b>" + newHeller + "</b> Heller<br>";
		flavor += "<b>" + newKreuzer + "</b> Kreuzer";
    ...
    function updating(newMoney){
       
		MoneyUpdate = newMoney;

		token.actor.update({
			'system.base.appearance.height': newMoney,
		});
	}
}