...
//Dialoge
	...
	inputDialog +="<h3><b>Herstellung einer neuen Rune</h3></b>";
	...
	inputDialog += divFlexStart + `
        ...
        `+ divFlexEnd + hr;		
	inputDialog += divFlexStart + "Ritualkenntnis: Runenzauberei<input id='RkWInput'" + divInputNumber  + "0'/>" + divFlexEnd + hr;
	inputDialog += "<h3><b>Reaktivierung einer bestehenden Rune</b></h3>";
	inputDialog += divFlexStart + `
            <label for="checkActivate">Rune erneut aktivieren</label><input type="checkbox" id="checkActivate" name="checkActivate" style="float:right">
        `+ divFlexEnd;
	inputDialog += divFlexStart + "TaP* aus Handwerksprobe<input id='renewTaP'" + divInputNumber  + "0'/>" + divFlexEnd;
	//###############################################################################################################################################################################################################################################	
...
//Probenwurf
    ...
    async function htmlCallback(html){
		...
		const renewTaPValue = Number(html.find("#renewTaP")[0]?.value || 0);							//TaP* aus vorhergegangenen Handwerksprobe	
		...
		const checkActivateInput = html.find("#checkActivate")[0].checked;								//passende Talentspezialisierung
				checkActivateMod = (checkActivateInput === true)? 1 : 0;										//Zuweisen eines Wertes für aktivierte Checkbox
		...
		TaW = TalentValue;
            ...
            win = Procedure + " eine <b>" + nameRune + "</b>";

			mod = complexity + holdTaPMod
			modOutput = (mod >=0)? "+" + mod: mod;

			eTaW = TaW + checkTSMod + (mhwInput*2);
			eTaWMod = eTaW - mod;
			resOne = Att1 - w1;
		...
		RkW = RkWValue;
			...
			if(checkActivateMod === 0){
				TaWboostMod = checkTaWboostMod;
			}else{
				TaWboostMod = Math.round(renewTaPValue/3);
			}
			
			mod2 = 0
			modOutput2 = (mod2 >=0)? "+" + mod2: mod2;

			eRkW = RkW + TaWboostMod + (holdTaPMod*2);
			eRkWMod = eRkW + mod2;
			...
			if(ritualResult < 0 && talentResult > 0){
				failOut2 = fail2;
			...
			//Chat Output
			flavor = "Runenzauberei:";
            if(checkActivateMod === 0){
				flavor += "<b> Erstellung einer " + nameRune + "</b>";
				...
				if(talentResult > 0){
					...
					flavor += "<b> Reaktivierung der " + nameRune + "</b>"
					flavor += "<br><b>KL</b>: " + w4 + "/" + "<b>" + cleverness + "</b>" + " (" + resFour + ")";
					flavor += "<br><b>IN</b>: " + w5 + "/" + "<b>" + intuition + "</b>" + " (" + resFive + ")";
					flavor += "<br><b>FF</b>: " + w6 + "/" + "<b>" + dexterity + "</b>" + " (" + resSix + ")";
					flavor += "<br>RkW: " + eRkW + " / " + modOutput2;
					...
					flavor += "<br>" + hr;
					if(ritualResult > 0){
						flavor += "<b>" + tokenName+ "</b> reaktiviert die <b>" + nameRune + "</b>:";
						flavor += "<br>" + RuneOutput + RuneOutput2 + "</br>";
					}
				}
            roll.toMessage ({
