...
//Dialoge
	...
	inputDialog += divFlexStart + `
        ...
        `+ divFlexEnd;
	inputDialog += divFlexStart + `
            <label for="checkMage">Repräsentation: Gildenmagier</label><input type="checkbox" id="checkMage" name="checkMage" style="float:right">
        `+ divFlexEnd + hr;
...
//Probenwurf
    ...
    async function htmlCallback(html){
		...
		const checkMageInput = html.find("#checkMage")[0].checked;						//Repräsentation:Gildenmagier
		...
		const ZfW = ElementarerDienerValue;
			...
			if(checkMage === true){
				mod = summonDiffMod - trueNameMod - ElementaristValue - GiftMod + AntiGiftMod - (doubledTimeMod + 1) + Math.round((halfTimeMod + reduceAsPMod)/2)
			}else{mod = summonDiffMod - trueNameMod - ElementaristValue - GiftMod + AntiGiftMod - doubledTimeMod + halfTimeMod + reduceAsPMod
			}
